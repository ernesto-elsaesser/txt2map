...
class TestEvaluator:
  ...
  def __init__(self):
    self.parser = Geoparser(use_large_model=False)
  ...
  def test_global_default_sense(self):
    ...
    self._test('Global - Default Sense', text, anns)
  ...
  def test_global_onto_sim(self):
    ...
    self._test('Global - Ontological Similarity', text, anns)
  ...
  def test_global_onto_sim_hard(self):
    ...
    anns = [('San Marcos', 0, 0, 5392368),
            ...
            ('America', 0, 0, 6252001)]
    self._test('Global - Ontological Similarity Hard', text, anns)
  ...
  def test_global_top_defaults(self):
    ...
    anns = [('France', 0, 0, 3017382),
            ...
            ('Mexico', 0, 0, 3996063),
            ('Uruguay', 0, 0, 3439705)]
    self._test('Global - Fixed Top-Level Senses', text, anns)
  ...
  def test_global_name_sim(self):
    ...
    self._test('Local - Special Characters', text, anns)

  def test_global_abbrevs(self):
    text = 'It\'s nice in Calif.'
    anns = [('Calif.', 0, 0, 5332921)]
    self._test('Global - Abbreviations', text, anns)
  ...
  def test_local_fuzzy(self):
    ...
    self._test('Local - Fuzzy Matching', text, anns)

  def _test(self, title, text, annotations):
    print(f'--- {title} ---')
